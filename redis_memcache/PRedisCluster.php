...
class PRedisCluster{    
     ...
     * @param string $port
     */
    private function init($host,$port='6379'){
      ...
      //  $_socket = $this->cluster[ self::$myself ]->socket ;
        $_socket = stream_socket_client( $host.':'.$port, $errno, $errstr, 30 );
        print_r($_socket);
        print_r(gettype($_socket));
        $redd = $this->connect("192.168.30.104");
        print_r($redd);echo "\n\n";
        //$_socket = $redd->socket;
        $reflector = new ReflectionObject ($redd);
        echo "\n",$reflector->name,"\n";
       // $Reflection = new ReflectionProperty(get_class($redd), 'a');
        print_r($reflector);echo "\n\n";
        //$_socket = $reflector->socket;
